...
from app.utils.annonations import API

YEAR          = "year"
MONTH         = "month"
DAY           = "day"
HOUR          = "hour"
HEAVEN_STEM   = "tian_gan"
EARTH_BRANCH  = "di_zhi"
...
class BaziPaipanEngine:
    ...
    @API
    def lunar_to_solar(self, year: int, month: int, day: int, is_leap_month: bool = False) -> SolarDate:
        """将农历转换为阳历"""
    ...
    def calculate_bazi(
        ...
        bazi_info = BaziInfo(
            year_pillar=self._create_pillar_info(year_gz.tg, year_gz.dz, day_stem),
            month_pillar=self._create_pillar_info(month_gz.tg, month_gz.dz, day_stem),
            day_pillar=self._create_pillar_info(day_gz.tg, day_gz.dz, day_stem),
            hour_pillar=self._create_pillar_info(hour_gz[0], hour_gz[1], day_stem)
        )

        # 计算大运
        bazi_info.destiny_cycle = self._calculate_dayun(day, hour, minute, gender, day_stem)
        
        return bazi_info

    def _create_pillar_info(self, stem_index: int, branch_index: int, day_stem: str) -> PillarInfo:
        """创建柱信息"""
        stem_name = self.TIAN_GAN_NAMES[stem_index]
        branch_name = self.DI_ZHI_NAMES[branch_index]

        hidden_stem_list = []
        for hidden_stem in BRANCH_HIDDEN_STEM[branch_name]:
            hidden_stem_list.append(HeavenStemInfo(
                name=hidden_stem,
    ...
    def _calculate_ten_gods(self, bazi: Dict, day_stem: str) -> Dict[str, Dict[str, str]]:
        ...
        for pillar in [YEAR, MONTH, DAY, HOUR]:
            ...
            # 计算天干十神
            stem = bazi[pillar][HEAVEN_STEM]
            result[pillar][TEN_GODS] = self._calculate_stem_ten_god(stem, day_stem)
            
            # 计算地支藏干的十神
            branch = bazi[pillar][EARTH_BRANCH]
            result[pillar][EARTH_BRANCH] = self._get_branch_ten_god(branch, day_stem)
    ...
    def _calculate_five_elements(self, bazi: Dict) -> List[str]:
        ...
        for pillar in [YEAR, MONTH, DAY, HOUR]:
            stem = bazi[pillar][HEAVEN_STEM]
            five_elements.append(self.HEAVEN_STEM_ELEMENT_MAP[stem])
    ...
    # 大运计算
    def _calculate_dayun(self, birth_day: sxtwl.Day, hour: int, minute: int, gender: str, day_stem: str) -> Dict:
        """
        计算大运
        ...
        # 判断大运顺逆
        is_forward = (gender == "male" and is_yang_year) or \
...
def main():
    try:
        ...
        for pillar in [YEAR, MONTH, DAY, HOUR]:
            print(f"{pillar}柱:")
            print(f"  天干: {bazi[pillar][HEAVEN_STEM]}")
            print(f"  地支: {bazi[pillar][EARTH_BRANCH]}")
