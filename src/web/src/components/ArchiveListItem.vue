<template>
  ...
  >
    <button
      class="flex min-w-0 flex-1 items-center justify-between overflow-hidden rounded-xl border border-[rgba(255,255,255,0.12)] bg-[rgba(18,20,28,0.55)] px-3 py-2 pr-2 text-left backdrop-blur-xl transition-all duration-200 hover:-translate-y-0.5 hover:border-[rgba(214,160,96,0.55)] hover:shadow-[0_22px_48px_rgba(0,0,0,0.5)]"
      :class="[
        isCurrent
    ...
    >
      <div class="min-w-0 flex-1 space-y-1.5">
        <div class="flex flex-wrap items-center gap-x-2 gap-y-1 text-left">
          <div class="min-w-0 max-w-full truncate text-md font-semibold text-[var(--accent-2)]">
            {{ entry.displayName }}
          </div>
          <div class="flex flex-wrap items-center gap-2">
            <span
              v-if="genderLabel(entry.gender ?? entry.chart?.gender)"
              class="text-sm font-normal text-[var(--muted)]"
            >
...
import archiveMoreIconUrl from '../assets/svg/archive_more.svg';
